const Basic6result = require("../models/Basic6result");
...
router.post("/", async (req, res) => {
  ...
  try {
    const ResultAlreadyExits = await Basic6result.findOne({
      year,
      term,
      classes,
      user,
    ...
    const newResult = new Basic6result({
...
router.get("/", async (req, res) => {
  try {
    const basic4results = await Basic6result.find({})
      .sort({ createdAt: -1 })
      .populate("user", [
        "firstName",
        "lastName",
...
router.get("/:id", async (req, res) => {
  try {
    const basic4result = await Basic6result.findById(req.params.id).populate(
      "user",
...
router.get("/results/:user/:year/:term/", async (req, res) => {
  try {
    ...
    const basic4result = await Basic6result.findOne({
      user,
      year,
...
router.put("/updateResultPosition/:id", async (req, res) => {
  ...
  try {
    const prenurseryresult = await Basic6result.findById(id);
...
router.put("/deactivateResultEdit", async (req, res) => {
  try {
    ...
    const result = await Basic6result.updateMany(
      { classes, year, term },
...
router.get("/:year/:term/", async (req, res) => {
  try {
    ...
    // Use the parameters to query the database
    const basic5result = await Basic6result.findOne({
      year,
