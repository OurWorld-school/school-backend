const Basic5Commulative = require("../models/Basic5Commulative");
...
router.post("/", async (req, res) => {
  ...
  try {
    const ResultAlreadyExits = await Basic5Commulative.findOne({
      year,
      term,
      classes,
      user,
    ...
    const newResult = new Basic5Commulative({
...
router.get("/", async (req, res) => {
  try {
    const basic1results = await Basic5Commulative.find({})
      .sort({ createdAt: -1 })
      .populate("user", [
        "firstName",
        "lastName",
...
router.get("/:id", async (req, res) => {
  try {
    const basic1result = await Basic5Commulative.findById(
      req.params.id
    ).populate("user", [
      "firstName",
      "lastName",
...
router.get("/results/:user/:year/:term/", async (req, res) => {
  try {
    ...
    const basic1result = await Basic5Commulative.findOne({
      user,
      year,
...
router.put("/updateResultPosition/:id", async (req, res) => {
  ...
  try {
    const prenurseryresult = await Basic5Commulative.findById(id);
...
router.put("/deactivateResultEdit", async (req, res) => {
  try {
    ...
    const result = await Basic5Commulative.updateMany(
      { classes, year, term },
...
router.get("/:year/:term/", async (req, res) => {
  try {
    ...
    // Use the parameters to query the database
    const basic5result = await Basic5Commulative.findOne({
      year,
