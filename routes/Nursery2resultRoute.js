...
router.put("/update/:id", async (req, res) => {
  ...
  try {
    const result = await Nursery2result.findById(id);
    ...
    updateSubject(SocialStudies, "SocialStudies");
    updateSubject(PVC, "PVC");
    updateSubject(CreativeArt, "CreativeArt");
    updateSubject(Writing, "Writing");
...
router.put("/deactivateResultEdit", async (req, res) => {
  try {
    ...
    const result = await Nursery2result.updateMany(
      { classes, year, term },
...
router.get("/:year/:term/", async (req, res) => {
  try {
    ...
    // Use the parameters to query the database
    const basic5result = await Nursery2result.findOne({
      year,
