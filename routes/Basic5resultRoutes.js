const Basic5result = require("../models/Basic5result");
...
router.post("/", async (req, res) => {
  ...
  try {
    const ResultAlreadyExits = await Basic5result.findOne({
      year,
      term,
      classes,
      user,
    ...
    const newResult = new Basic5result({
...
router.get("/", async (req, res) => {
  try {
    const basic4results = await Basic5result.find({})
      .sort({ createdAt: -1 })
      .populate("user", [
        "firstName",
        "lastName",
...
router.get("/:id", async (req, res) => {
  try {
    const basic4result = await Basic5result.findById(req.params.id).populate(
      "user",
...
router.get("/results/:user/:year/:term/", async (req, res) => {
  try {
    ...
    const basic4result = await Basic5result.findOne({
      user,
      year,
...
router.put("/updateResultPosition/:id", async (req, res) => {
  ...
  try {
    const prenurseryresult = await Basic5result.findById(id);
...
router.put("/update/:id", async (req, res) => {
  ...
  try {
    const result = await Basic5result.findById(id);
...
router.put("/deactivateResultEdit", async (req, res) => {
  try {
    ...
    const result = await Basic5result.updateMany(
      { classes, year, term },
...
router.get("/:year/:term/", async (req, res) => {
  try {
    ...
    // Use the parameters to query the database
    const basic5result = await Basic5result.findOne({
      year,
