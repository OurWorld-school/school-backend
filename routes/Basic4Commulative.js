const Basic4Commulative = require("../models/Basic4Commulative");
...
router.post("/", async (req, res) => {
  ...
  try {
    const ResultAlreadyExits = await Basic4Commulative.findOne({
      year,
      term,
      classes,
      user,
    ...
    const newResult = new Basic4Commulative({
    ...
    await User.findByIdAndUpdate(userId, {
      $push: { basic4commulative: newResult._id },
    });
...
router.get("/", async (req, res) => {
  try {
    const basic1results = await Basic4Commulative.find({})
      .sort({ createdAt: -1 })
      .populate("user", [
        "firstName",
        "lastName",
...
router.get("/:id", async (req, res) => {
  try {
    const basic1result = await Basic4Commulative.findById(
      req.params.id
    ).populate("user", [
      "firstName",
      "lastName",
...
router.get("/results/:user/:year/:term/", async (req, res) => {
  try {
    ...
    const basic1result = await Basic4Commulative.findOne({
      user,
      year,
...
router.put("/updateResultPosition/:id", async (req, res) => {
  ...
  try {
    const prenurseryresult = await Basic4Commulative.findById(id);
...
router.put("/deactivateResultEdit", async (req, res) => {
  try {
    ...
    const result = await Basic4Commulative.updateMany(
      { classes, year, term },
...
router.get("/:year/:term/", async (req, res) => {
  try {
    ...
    // Use the parameters to query the database
    const basic5result = await Basic4Commulative.findOne({
      year,
