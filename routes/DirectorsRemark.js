const DirectorsResultRemark = require("../models/DirectorsResultRemark");
...
router.post("/", async (req, res) => {
  ...
  try {
    const existingGrade = await DirectorsResultRemark.findOne({
      gradeName,
      schoolName: schoolId,
    });
    ...
    //create new Director result grade

    const newGrade = new DirectorsResultRemark({
...
router.get("/", async (req, res) => {
  try {
    const grades = await DirectorsResultRemark.find({})
      .sort({ createdAt: -1 })
...
router.get("/:id", async (req, res) => {
  try {
    const grades = await DirectorsResultRemark.findById(req.params.id).populate(
      "schoolName",
      ["name"]
    );
...
router.delete("/delete/:id", async (req, res) => {
  try {
    const grades = await DirectorsResultRemark.findByIdAndDelete(req.params.id);
    if (grades) {
...
router.put("/update/:id", async (req, res) => {
  ...
  const { gradeName, gradeRemark } = req.body;
  try {
    const grades = await DirectorsResultRemark.findById(id);
