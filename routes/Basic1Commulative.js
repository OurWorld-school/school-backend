const Basic1Commulative = require("../models/Basic1Commulative");
...
router.post("/", async (req, res) => {
  ...
  try {
    const ResultAlreadyExits = await Basic1Commulative.findOne({
      year,
      term,
      classes,
      user,
    ...
    const newResult = new Basic1Commulative({
...
router.get("/", async (req, res) => {
  try {
    const basic1results = await Basic1Commulative.find({})
      .sort({ createdAt: -1 })
      .populate("user", [
        "firstName",
        "lastName",
...
router.get("/:id", async (req, res) => {
  try {
    const basic1result = await Basic1Commulative.findById(
      req.params.id
    ).populate("user", [
      "firstName",
      "lastName",
...
router.get("/results/:user/:year/:term/", async (req, res) => {
  try {
    ...
    const basic1result = await Basic1Commulative.findOne({
      user,
      year,
...
router.put("/updateResultPosition/:id", async (req, res) => {
  ...
  try {
    const prenurseryresult = await Basic1Commulative.findById(id);
...
router.put("/deactivateResultEdit", async (req, res) => {
  try {
    ...
    const result = await Basic1Commulative.updateMany(
      { classes, year, term },
...
router.get("/:year/:term/", async (req, res) => {
  try {
    ...
    // Use the parameters to query the database
    const basic5result = await Basic1Commulative.findOne({
      year,
