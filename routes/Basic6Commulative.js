const CommunativeBasic6result = require("../models/CommunativeBasic6result");
...
router.post("/", async (req, res) => {
  ...
  try {
    const ResultAlreadyExits = await CommunativeBasic6result.findOne({
      year,
      term,
      classes,
      user,
    ...
    const newResult = new CommunativeBasic6result({
    ...
    await User.findByIdAndUpdate(userId, {
      $push: { basic1commulative: newResult._id },
    });
...
router.get("/", async (req, res) => {
  try {
    const basic1results = await CommunativeBasic6result.find({})
      .sort({ createdAt: -1 })
      .populate("user", [
        "firstName",
        "lastName",
...
router.get("/:id", async (req, res) => {
  try {
    const basic1result = await CommunativeBasic6result.findById(
      req.params.id
    ).populate("user", [
      "firstName",
      "lastName",
...
router.get("/results/:user/:year/:term/", async (req, res) => {
  try {
    ...
    const basic1result = await CommunativeBasic6result.findOne({
      user,
      year,
...
router.put("/updateResultPosition/:id", async (req, res) => {
  ...
  try {
    const prenurseryresult = await CommunativeBasic6result.findById(id);
...
router.put("/deactivateResultEdit", async (req, res) => {
  try {
    ...
    const result = await CommunativeBasic6result.updateMany(
      { classes, year, term },
...
router.get("/:year/:term/", async (req, res) => {
  try {
    ...
    // Use the parameters to query the database
    const basic5result = await CommunativeBasic6result.findOne({
      year,
