const Basic3Commulative = require("../models/Basic3Commulative");
...
router.post("/", async (req, res) => {
  ...
  try {
    const ResultAlreadyExits = await Basic3Commulative.findOne({
      year,
      term,
      classes,
      user,
    ...
    const newResult = new Basic3Commulative({
    ...
    await User.findByIdAndUpdate(userId, {
      $push: { basic3commulative: newResult._id },
    });
...
router.get("/", async (req, res) => {
  try {
    const basic1results = await Basic3Commulative.find({})
      .sort({ createdAt: -1 })
      .populate("user", [
        "firstName",
        "lastName",
...
router.get("/:id", async (req, res) => {
  try {
    const basic1result = await Basic3Commulative.findById(
      req.params.id
    ).populate("user", [
      "firstName",
      "lastName",
...
router.get("/results/:user/:year/:term/", async (req, res) => {
  try {
    ...
    const basic1result = await Basic3Commulative.findOne({
      user,
      year,
...
router.put("/updateResultPosition/:id", async (req, res) => {
  ...
  try {
    const prenurseryresult = await Basic3Commulative.findById(id);
...
router.put("/deactivateResultEdit", async (req, res) => {
  try {
    ...
    const result = await Basic3Commulative.updateMany(
      { classes, year, term },
...
router.get("/:year/:term/", async (req, res) => {
  try {
    ...
    // Use the parameters to query the database
    const basic5result = await Basic3Commulative.findOne({
      year,
